...
module.exports = {
    ...
    async run (bot, message, args) {
        ...
        if(!channelID) return message.reply("Please specify a valid channel you want the embed to be sent in!\n `Example: y!reactionrole #<channel> <Your Description>`")

        const desc = args.slice(1).join(" ")
        if(!desc) return message.reply("Please add a valid description!\n `Example: y!reactionrole #<channel> <Your Description>`")


        const Role1 = message.guild.roles.cache.find(role => role.name === "TESTERS")
        const Role2 = message.guild.roles.cache.find(role => role.name === "level5")
